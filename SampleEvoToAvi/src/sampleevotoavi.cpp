...
//used to transform evo mat to opencv format
#include "evo_matconverter.h"
//used to save avi file
...
int main(int argc, char* argv[])
{
	...
	std::cout << "usage: SampleEvoToAvi [. evo file, (only file name, without .evo)] [do rectify or not(default = 0, when value = 1, do rectify)][start index,default = 0][end index]" << std::endl;
	...
	if (camera.open(file_name.c_str()) == evo::RESULT_CODE_OK)
	{
		...
		while (camera.getCurrentEvoPosition() < lastIndex && isRunning == true)
		{
